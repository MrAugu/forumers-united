...
class Suggest extends Command {
  ...
  async run (message, args, level, reply) { // eslint-disable-line no-unused-vars
    ...
    const m = await this.client.channels.get("611931175514144768").send(`**Feedback:**\`\`\`${feedback}\`\`\`\nFeedback by ${message.author}.\nID: **#${id}**`);
    message.delete();
    message.author.send(`Feedback with id **#${id}** has been sent to our team. Thank you for your feedback and helping us making the server a better place.`);
  }
}
